...
export class AuthController {
  ...
  @Post('verify-otp') @Throttle({ default: { limit: 5, ttl: 60_000 } }) verifyOtp(@Body() body: VerifyOtpDto) { return this.auth.verifyOtp(body); }
  @Post('refresh') refresh(@Body() body: RefreshDto) { return this.auth.refresh(body); }
  @Post('logout') logout(@Body() body: RefreshDto) { return this.auth.logout(body); }
}
