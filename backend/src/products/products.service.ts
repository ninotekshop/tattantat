...
export class ProductsService {
  ...
  async create(sellerId: string, body: CreateProductDto) {
    ...
    const id = await this.database.transaction(async (client) => {
      // Serialize listing creation for this seller, so parallel requests cannot
      // both pass the plan limit before either inserts its product.
      ...
      const created = await client.query<{ id: string }>(
        `INSERT INTO products (seller_id, category_id, title, slug, description, price, condition, status, published_at) VALUES ($1,$2,$3,$4,$5,$6,$7::product_condition,'ACTIVE',NOW()) RETURNING id`,
        [sellerId, body.categoryId, body.title.trim(), slug, body.description?.trim() ?? null, body.price, body.condition],
      );
